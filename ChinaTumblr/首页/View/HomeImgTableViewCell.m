...
@implementation HomeImgTableViewCell
...
-(id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self=[super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [_backView addSubview:_gerenBtn];
        ...
        [_backView addSubview:_showImgView];
...
//        _showImgView.clipsToBounds = YES;
        ...
        UITapGestureRecognizer *tap1 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(showImgClick:)];
        [self.showImgView addGestureRecognizer:tap1];
        
        
        
        // 图片张数
        _imgNumLb = [[UILabel alloc] init];
        [_showImgView addSubview:_imgNumLb];
        [_imgNumLb mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(_showImgView).with.offset(-8);
            make.top.equalTo(_showImgView).with.offset(8);
        }];
        _imgNumLb.textColor = FUIColorFromRGB(0xffffff);
        _imgNumLb.font = [UIFont systemFontOfSize:13];
        ...
        // 喜欢按钮
        [_backView addSubview:self.praiseBtn];
        [_backView addSubview:self.coverBtn];
        [_backView insertSubview:self.coverBtn belowSubview:self.praiseBtn];
        [_backView addSubview:self.cancelPraiseImg];
        ...
        [_backView addSubview:_pinglunBtn];
        [_pinglunBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(self.praiseBtn);
//            make.left.equalTo(self.praiseBtn.mas_right).with.offset(0.03 * Width);
        ...
        [_shareBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(self.praiseBtn);
//            make.left.equalTo(_pinglunBtn.mas_right).with.offset(0.03 * Width - 1);
        ...
        [_backView addSubview:_pinglunNumBtn];
        ...
        [_backView addSubview:_dongtaiBtn];
...
- (void) giveArrForbiaoqian:(NSArray *)arr andNavIndex:(NSInteger)navIndex{
    ...
    [_backView addSubview:jinghaoLb];
...
// 展示图片点击事件
- (void) showImgClick:(UIImageView *)sender {
    if (self.showImgViewClick) {
        self.showImgViewClick();
    }
}
// 头像点击事件
...
- (UIButton*)praiseBtn
{
    if (!_praiseBtn) {
        ...
        _praiseBtn.frame = CGRectMake(0.01625 * Width, _showImgView.frame.origin.y + _showImgView.frame.size.height + 0.013125 * Width, 20 + 0.03 * Width, 20 + + 0.03 * Width);
