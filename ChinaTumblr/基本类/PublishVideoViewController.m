...
//  ChinaTumblr
//
//  Created by 张宗琦 on 2017/8/9.
//  Copyright © 2017年 张宗琦. All rights reserved.
//

#import "PublishVideoViewController.h"
#import "CustomCamera.h" // 摄像页面
...
@interface PublishVideoViewController ()<UITextViewDelegate,ZFPlayerDelegate,UIActionSheetDelegate> {
    
    // 拍摄按钮
    UIButton *paisheBtn;
    
    UIImage *_videoImg; // 视频第一帧
...
- (void) initDataSource {
    ...
    [_dicData setValue:@"2" forKey:@"type"];
...
- (void) layoutViews {
    ...
    [whiteBackView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(@(H * 0.7));
    }];
    whiteBackView.backgroundColor = [UIColor whiteColor];
    whiteBackView.tag = 1234;
    ...
    // 拍摄按钮
    paisheBtn = [[UIButton alloc] init];
    [self.view addSubview:paisheBtn];
    [paisheBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.view);
        make.bottom.equalTo(whiteBackView).with.offset(- (H * 0.7 - W * 0.4) * 0.5);
        make.height.equalTo(@(W * 0.2));
        make.width.equalTo(@(W * 0.2));
    }];
    [paisheBtn setTitle:@"拍摄" forState:UIControlStateNormal];
    paisheBtn.titleLabel.font = [UIFont systemFontOfSize:14];
    [paisheBtn setTitleColor:FUIColorFromRGB(0xfeaa0a) forState:UIControlStateNormal];
    paisheBtn.layer.cornerRadius = W * 0.1;
    paisheBtn.layer.borderColor = FUIColorFromRGB(0xfeaa0a).CGColor;
    paisheBtn.layer.borderWidth = 1.f;
    [paisheBtn addTarget:self action:@selector(paisheBtnClick:) forControlEvents:UIControlEventTouchUpInside];
...
//    lbFenge2.backgroundColor = FUIColorFromRGB(0xeeeeee);
    ...
    [quanxianBtn setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
...
// 拍摄点击事件
- (void) paisheBtnClick:(UIButton *)btn {
...
- (void) publishBtnClick {
    ...
    if ([[[_dicData objectForKey:@"content"] stringByReplacingOccurrencesOfString:@" "  withString:@""] isEqualToString:@""]) {
    ...
    }else if ([[user objectForKey:@"vedioURL"] length] < 1 && _model == nil){
        
        // 视频为空
        [MBHUDView hudWithBody:@"视频为空" type:MBAlertViewHUDTypeLabelIcon hidesAfter:1.f backGroundColorWithWhite:0.0 show:YES];
        ...
        if (_model == nil) {
            // 第一次发布
            NSDictionary *dicDataForVideo = @{@"tk":userJiaMiArr[0],@"key":userJiaMiArr[1],@"cg":userJiaMiArr[2]};
            
            NSLog(@"dicDataForVideo:%@",dicDataForVideo);
            
            // 发布图片网络请求
            [http PostVideoToServerWithUserInfo:dicDataForVideo andImg:_videoImg andVideo:[NSURL URLWithString:[user objectForKey:@"vedioURL"]] Success:^(id arrForDetail) {
                ...
                if ([arrForDetail isEqualToString:@"false"]) {
                    ...
                    // 编号
                    [_dicData setValue:arrForDetail forKey:@"files"];
                    ...
                    [http PostAddNoteWithDic:dicData111 Success:^(id userInfo) {
                        ...
                        if ([userInfo isEqualToString:@"0"]) {
                            ...
                            // 发帖失败
                            [MBHUDView hudWithBody:@"发帖失败" type:MBAlertViewHUDTypeLabelIcon hidesAfter:1.f backGroundColorWithWhite:0.0 show:YES];
...
//                            [notiCenter postNotificationName:@"fatieChengGong" object:@"20"];
            ...
            [_dicData setValue:[_model.files[0] valueForKey:@"id"] forKey:@"files"];
            // 获取到用户相关的加密信息
            NSString *dataStrJiaMi = [[MakeJson createJson:_dicData] AES128EncryptWithKey:userJiaMiArr[3]];
...
- (void) viewWillAppear:(BOOL)animated {
    ...
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    if ([[user objectForKey:@"vedioURL"] length] < 1) {
        ...
        if (_model == nil) {
            ...
            // 父视图
            UIView *whiteView = [self.view viewWithTag:1234];
            UIView *fatherView1 = [[UIView alloc] init];
            [whiteView addSubview:fatherView1];
            [fatherView1 mas_makeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo(whiteView).with.offset(0.2 * W);
                make.left.equalTo(whiteView);
                make.right.equalTo(whiteView);
                make.height.equalTo(@(H * 0.7 - 0.2 * W));
                make.width.equalTo(@(W));
            }];
            playerModel.fatherView = fatherView1;
            playerModel.videoURL = [NSURL URLWithString:[_model.files[0] valueForKey:@"path"]];
            playerModel.placeholderImage = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[_model.files[0] valueForKey:@"path_cover"]]]];
            [self.playerView playerControlView:controlView playerModel:playerModel];
            // 隐藏拍摄按钮
            paisheBtn.hidden = YES;
            // delegate
        ...
        // 视频第一帧
        _videoImg = [self getVideoPreViewImage:[NSURL URLWithString:[user objectForKey:@"vedioURL"]]];
...
//        }];
        ...
        playerModel.fatherView = fatherView1;
        playerModel.videoURL = [NSURL URLWithString:[user objectForKey:@"vedioURL"]];
        playerModel.placeholderImage = _videoImg;
