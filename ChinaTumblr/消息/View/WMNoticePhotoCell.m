...
#import "WMNoticePhotoCell.h"

@implementation WMNoticePhotoCell
...
-(id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self=[super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        // 配图
        _peituImgView = [[UIImageView alloc] init];
        [self addSubview:_peituImgView];
        [_peituImgView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        _peituImgView.layer.cornerRadius = 5;
        _peituImgView.clipsToBounds = YES;
        _peituImgView.userInteractionEnabled = YES;
        UITapGestureRecognizer *tap1 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tieziImgClick:)];
        [_peituImgView addGestureRecognizer:tap1];
        [_peituImgView setContentMode:UIViewContentModeScaleAspectFill];
        _peituImgView.clipsToBounds = YES;
        
        // 播放图标
        _playImgView = [[UIImageView alloc] init];
        [_peituImgView addSubview:_playImgView];
        [_playImgView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.center.equalTo(_peituImgView);
        ...
        [_noticeTipLb mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.right.equalTo(self).with.offset(-10 - 72 - 10);
...
// 帖子点击事件
- (void) tieziImgClick:(UIImageView *)sender {
    if (self.tieziViewClick) {
        self.tieziViewClick();
    }
}
// 头像点击事件
