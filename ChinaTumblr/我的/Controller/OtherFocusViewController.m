...
#import "OtherFocusViewController.h"
...
@interface OtherFocusViewController () <UITableViewDelegate, UITableViewDataSource> {
    ...
    NSArray *_arrForAllLikeUserId;// 所有喜欢的人id
}
...
@implementation OtherFocusViewController
...
- (void) initDataSource {
    ...
    if (pageStart == 0) {
        ...
        NSDictionary *dicdata = @{@"uid":_userId1,@"pageSize":[NSString stringWithFormat:@"%ld",pageSize]};
        ...
        NSDictionary *dicdata = @{@"uid":_userId1,@"pageSize":[NSString stringWithFormat:@"%ld",pageSize],@"startTime":model.followTime,@"lastUserId":model.id1};
        NSString *strData = [[MakeJson createJson:dicdata] AES128EncryptWithKey:userJiaMiArr[3]];
        NSDictionary *dicForData = @{@"tk":userJiaMiArr[0],@"key":userJiaMiArr[1],@"cg":userJiaMiArr[2],@"data":strData};
        // 测试获取关注用户的信息
        [http PostGetFollowUserInfoListForUcenterWithDic:dicForData Success:^(id userInfo) {
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (scrollView.contentOffset.y < 0) {
        ...
        // 在消息中心发布自己的消息
        [notiCenter postNotificationName:@"xiugaitableViewScrollForOther67" object:@"67"];
    }
}
