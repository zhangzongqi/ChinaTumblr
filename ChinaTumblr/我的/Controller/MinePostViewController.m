...
#import "MinePostViewController.h"
...
@interface MinePostViewController ()<UITableViewDelegate,UITableViewDataSource,ZFPlayerDelegate,CAAnimationDelegate,UIActionSheetDelegate,UIAlertViewDelegate> {
...
@implementation MinePostViewController
...
- (void)viewDidLoad {
    ...
    _canScroll = NO;
    // 用单例
    [USER setValue:@"NO" forKey:@"_tableViewCanScroll"];
...
- (void)initData {
    ...
    if (pageStart == 0) {
        ...
        NSLog(@"***dicDataJiaMi%@",dicDataJiaMi);
        ...
        NSDictionary *dataDic = @{@"uid":@"",@"pageSize":[NSString stringWithFormat:@"%ld",pageSize],@"lastNoteId":model.id1,@"startTime":model.update_time};
        NSString *strDataJiaMi = [[MakeJson createJson:dataDic] AES128EncryptWithKey:arrUserJiaMi[3]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([model.type isEqualToString:@"1"]) {
        ...
        [cell.rightBtn setTitleColor:FUIColorFromRGB(0x212121) forState:UIControlStateNormal];
...
//    NSLog(@"这是第%ld行",path.row);
    ...
    if (scrollView.contentOffset.y < 0) {
        ...
        // 在消息中心发布自己的消息
        [notiCenter postNotificationName:@"xiugaitableViewScrollForOther" object:@"16"];
    }
}
...
- (void)listen:(NSNotification *)noti {
    ...
    if ([strNoti isEqualToString:@"11"]) {
        ...
        _canScroll = YES;
        NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
        [user setValue:@"YES" forKey:@"_tableViewCanScroll"];
    ...
    if (_tableViewArr.count > 0) {
        for (int i = 0; i < DelTieZiForMineArr.count; i++) {
            ...
            for (int j = 0; j < _tableViewArr.count; j++) {
                ...
                SearchTieZiWithKeyWordModel *tempModel = _tableViewArr[j];
        ...
        for (int i = 0; i < LoveTieZiForReviseMineArr.count; i++) {
            ...
            for (int j = 0; j < _tableViewArr.count; j++) {
                ...
                if ([tempModel.id1 isEqualToString:LoveTieZiForReviseMineArr[i]]) {
                    ...
                    SearchTieZiWithKeyWordModel *model = _tableViewArr[j];
                    model.isLike = @"YES";
                }
            }
