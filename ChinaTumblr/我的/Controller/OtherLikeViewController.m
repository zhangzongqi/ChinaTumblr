...
#import "OtherLikeViewController.h"
...
#import "OtherMineViewController.h" // 别人的主页
...
@interface OtherLikeViewController ()<UITableViewDelegate,UITableViewDataSource,ZFPlayerDelegate,CAAnimationDelegate,UIActionSheetDelegate> {
...
@implementation OtherLikeViewController
...
- (void)initData {
    ...
    if (pageStart == 0) {
        ...
        NSArray *arrUserJiaMi = [GetUserJiaMi getUserTokenAndCgAndKey];
        NSDictionary *dataDic = @{@"uid":_userId1,@"pageSize":[NSString stringWithFormat:@"%ld",pageSize],@"startTime":model.love_time,@"lastNoteId":model.id1};
        NSString *strDataJiaMi = [[MakeJson createJson:dataDic] AES128EncryptWithKey:arrUserJiaMi[3]];
...
- (void) layoutViews {
    ...
    self.tableView.estimatedRowHeight = 390.0f;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([model.type isEqualToString:@"1"]) {
        ...
        // 通过不同标识创建cell实例
        HomeImgTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        // 判断为空进行初始化  --（当拉动页面显示超过主页面内容的时候就会重用之前的cell，而不会再次初始化）
        if (!cell) {
            
            cell = [[HomeImgTableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:CellIdentifier];
        }
        ...
        NSString *CellIdentifier = [NSString stringWithFormat:@"cell%ld%ld",indexPath.section,indexPath.row];
        // 通过不同标识创建cell实例
        HomeTextCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        // 判断为空进行初始化  --（当拉动页面显示超过主页面内容的时候就会重用之前的cell，而不会再次初始化）
        if (!cell) {
            
            cell = [[HomeTextCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:CellIdentifier];
        }
        ...
        NSString *CellIdentifier = [NSString stringWithFormat:@"cell%ld%ld",indexPath.section,indexPath.row];
        // 通过不同标识创建cell实例
        HomeVideoCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        // 判断为空进行初始化  --（当拉动页面显示超过主页面内容的时候就会重用之前的cell，而不会再次初始化）
        if (!cell) {
            
            cell = [[HomeVideoCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:CellIdentifier];
        }
...
// 举报
- (void) jubaoBtnBy:(UserLikeTieZiListModel *)model {
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (scrollView.contentOffset.y < 0) {
        ...
        // 在消息中心发布自己的消息
        [notiCenter postNotificationName:@"xiugaitableViewScrollForOther68" object:@"68"];
    }
}
...
- (void) viewWillAppear:(BOOL)animated {
    ...
    // 修改滚动
    [notiCenter addObserver:self selector:@selector(listen:) name:@"xiugaitableViewScrollForOther66" object:@"66"];
    // 侧栏退出
