...
- (void)layoutSubviews{
    ...
    for (UIButton *btn in self.itemBtns) {
        ...
        btn.centerX = lastX - btn.width/2;
        
        lastX += btn.width + caculateMargin;
    }
    
    
    // 自己加上去的代码，用于约束三个按钮的位置
    if (_itemBtns.count == 3) {
        self.itemBtns[0].y = 0;
        self.itemBtns[0].centerX = self.bounds.size.width / 3 / 2;
        self.itemBtns[1].y = 0;
        self.itemBtns[1].centerX = self.bounds.size.width / 3 +  self.bounds.size.width / 3 / 2;
        self.itemBtns[2].y = 0;
        self.itemBtns[2].centerX = self.bounds.size.width / 3 * 2 +  self.bounds.size.width / 3 / 2;
    }else if (_itemBtns.count == 2) {
        self.itemBtns[0].y = 0;
        self.itemBtns[0].centerX = self.bounds.size.width / 2 / 2;
        self.itemBtns[1].y = 0;
        self.itemBtns[1].centerX = self.bounds.size.width / 2 +  self.bounds.size.width / 2 / 2;
    }else {
        self.itemBtns[0].y = 0;
        self.itemBtns[0].centerX = self.bounds.size.width / 2;
    }
...
//    self.indicatorView.height = self.config.indicatorH;
    ...
    if (iPhone5S) {
        ...
        self.indicatorView.y = self.height - self.indicatorView.height - 4;
        
    }else if (iPhone6S) {
        
        self.indicatorView.y = self.height - self.indicatorView.height - 6;
        
    }else if (iPhone6SP) {
        
        self.indicatorView.y = self.height - self.indicatorView.height - 8;
