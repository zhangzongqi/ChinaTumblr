...
#define kMaxVideoLength 10  //设置拍摄时间
...
//    NSString转NSUrl：NSString *str = @"www.ngmob.com";
//    
//       NSURL *URL = [NSURL URLWithString:str];
//    
//    
//    
//    NSUrl转NSString：NSString *str1 = [URL absoluteString];
    
    // 创建用户单利
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    [user setValue:[outputFileURL absoluteString]  forKey:@"vedioURLtemp"];
...
- (void)compression {
    ...
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    NSURL *videoUrl = [NSURL URLWithString:[user objectForKey:@"vedioURLtemp"]];
        ...
        if (session1.status==AVAssetExportSessionStatusCompleted) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [assetsLibrary writeVideoAtPathToSavedPhotosAlbum:CompressURL completionBlock:^(NSURL *assetURL, NSError *error) {
                    ...
                    // 保存单利
                    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
                    [user setValue:[CompressURL absoluteString] forKey:@"vedioURL"];
...
- (void) viewWillDisappear:(BOOL)animated {
    ...
    // 移除临时的保存
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    [user removeObjectForKey:@"vedioURLtemp"];
}
