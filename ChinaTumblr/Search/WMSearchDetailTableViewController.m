//
//  WMSearchDetailTableViewController.m
//  ChinaTumblr
//
...
#import "WMSearchDetailTableViewController.h"
...
@interface WMSearchDetailTableViewController ()<UITableViewDelegate,UITableViewDataSource,ZFPlayerDelegate,CAAnimationDelegate,UIActionSheetDelegate> {
...
@implementation WMSearchDetailTableViewController
...
- (void) layoutViews {
    ...
    // 列表
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, W, H - 64 - 32) style:UITableViewStylePlain];
    ...
    self.tableView.estimatedRowHeight = 350.0f;
...
- (void)initData {
    ...
    NSDictionary *dicData = @{@"keyword":_guanjianciStr,@"pageStart":[NSString stringWithFormat:@"%ld",pageStart],@"pageSize":[NSString stringWithFormat:@"%ld",pageSize],@"orderBy":_oderByStr};
    NSString *strJiaMi = [[MakeJson createJson:dicData] AES128EncryptWithKey:arrUserJiaMi[3]];
    
    // dic
    NSDictionary *dicDataForJiaMi = @{@"tk":arrUserJiaMi[0],@"key":arrUserJiaMi[1],@"cg":arrUserJiaMi[2],@"data":strJiaMi};
    ...
    if (pageStart == 0) {
        ...
        [http PostGetNoteListByKeywordWithDataDic:dicDataForJiaMi Success:^(id userInfo) {
            ...
            if ([[userInfo valueForKey:@"kwId"] isEqualToString:@"flase"]) {
                ...
                // 清空数组，拿到新数据
                [_tableViewArr removeAllObjects];
                NSArray *arr = [userInfo valueForKey:@"noteList"];
                NSMutableArray *arrSearchLabelKeyWordList = [SearchTieZiWithKeyWordModel arrayOfModelsFromDictionaries:arr error:nil];
                _tableViewArr = [NSMutableArray arrayWithArray:arrSearchLabelKeyWordList];
        ...
        [http PostGetNoteListByKeywordWithDataDic:dicDataForJiaMi Success:^(id userInfo) {
            ...
            if ([[userInfo valueForKey:@"kwId"] isEqualToString:@"flase"]) {
                ...
                NSArray *arr = [userInfo valueForKey:@"noteList"];
                NSMutableArray *arrSearchLabelKeyWordList = [SearchTieZiWithKeyWordModel arrayOfModelsFromDictionaries:arr error:nil];
                [_tableViewArr addObjectsFromArray:arrSearchLabelKeyWordList];
...
// 修改用户的关注状态
- (void) reviseUserFollowStatus {
    
    HttpRequest *http = [[HttpRequest alloc] init];
    
    NSArray *arrUserJiaMi = [GetUserJiaMi getUserTokenAndCgAndKey];
    NSDictionary *arrForGetAllLikeUserId = @{@"tk":arrUserJiaMi[0],@"key":arrUserJiaMi[1],@"cg":arrUserJiaMi[2]};
    
    [http PostGetAllFollowUserIdListWithDic:arrForGetAllLikeUserId Success:^(id userInfo) {
...
//        // 通过不同标识创建cell实例
//        SearchImgDetailTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
//        // 判断为空进行初始化  --（当拉动页面显示超过主页面内容的时候就会重用之前的cell，而不会再次初始化）
//        if (!cell) {
//            cell = [[SearchImgDetailTableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:CellIdentifier];
//        }
...
// 头像点击block触发事件
- (void) iconImgViewBy:(SearchTieZiWithKeyWordModel *)model {
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    // 拿到用户id
    NSDictionary *dicForUserInfo = [user objectForKey:@"userInfo"];
    if ([model.uid isEqualToString:[dicForUserInfo valueForKey:@"id"]]) {
...
- (void) viewWillAppear:(BOOL)animated {
    ...
    if (isFirstComeIn == YES) {
        isFirstComeIn = NO;
    }else {
        // 修改用户关注状态
        [self reviseUserFollowStatus];
    }
